...
interface APIUploadData {
  ...
  CNPJUsuario: string; // string number
  isCPF: boolean;
  NomeFantasia: string;
  NomeUsuario: string;
  ...
  enderecoUsuario: {
    ...
    Logradouro: string;
    NumeroLogradouro: string; // string number
...
const ComposedSelect: React.FC<ComposedSelectData> = ({ name, ...rest }) => {
  return (
    <div>
      <Select name={name} {...rest} styles={{ borderRadius: 4, height: 40 }} />
    </div>
  );
};

const PessoaJuridica = ({ updateTypeSignUp }: { updateTypeSignUp: () => void }) => {
      ...
      api
        ...
        .then(response => {
          if (response.data.Mensagem === 'Sucesso') {
            ...
            addAlert({
              title: 'Erro',
              description: 'Erro ao cadastrar usuário. ' + response.data.Mensagem,
              type: 'error',
            });
          }
          window.scrollTo(0, 0);
  ...
  const handleSubmit = useCallback(
      ...
      try {
        ...
        const schema = Yup.object()
          ...
          .shape({
            ...
            CNPJUsuario: Yup.string()
              .required('Informe o CNPJ')
              .transform(value => value.replace(/\D/g, ''))
              .test('valid_CNPJ', 'CNPJ inválido', value => validateCNPJ(value || '')),
            // #endregion
            // #region NomeFantasia validation
    ...
    [state, formRef, addAlert, handleSignUp],
  );
            ...
            <FormContainer>
              ...
              <FormRow className="mt-2">
                ...
                <div className="selectUF inputMargin">
                  <ComposedSelect name="SiglaUF" defaultValue="MG">
